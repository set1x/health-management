...
public interface ExerMapper {
  ...
  @Select(
      "SELECT ExerciseItemID FROM exerciseitem WHERE UserID = #{userID} AND RecordDate = #{recordDate} AND ExerciseType = #{exerciseType} AND DurationMinutes = #{durationMinutes} AND EstimatedCaloriesBurned = #{estimatedCaloriesBurned}")
