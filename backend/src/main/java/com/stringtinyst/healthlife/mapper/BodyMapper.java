...
public interface BodyMapper {
    ...
    @Insert("INSERT INTO bodymetrics(UserID, HeightCM, WeightKG, RecordDate) VALUES  (#{userID}, #{heightCM}, #{weightKG}, #{recordDate})")
    ...
    @Update("UPDATE bodymetrics SET HeightCM = #{heightCM}, WeightKG = #{weightKG}, RecordDate = #{recordDate} WHERE BodyMetricID = #{bodyMetricID}")
    public void updateBody(Body body);

    @Select("SELECT BodyMetricID FROM bodymetrics WHERE UserID = #{userID} AND HeightCM = #{heightCM} AND WeightKG = #{weightKG} AND RecordDate = #{recordDate}")
