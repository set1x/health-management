...
- **连接管理**：
  - 前端使用智能重连机制，网络中断时自动重试
  - 最多重试 5 次，失败后显示错误提示
  - 连接状态实时反馈：连接中、已连接、连接失败
  - 重连时自动携带对话历史，保持上下文连贯
  - 用户主动取消（AbortController）时不触发重连

- **流转超时**：若 60 秒内后端未能继续发送分片，将自动终止本次响应并推送 `{"content":"AI 服务响应超时，请稍后重试"}`，前端会触发自动重连
- **AI 不可用**：当模型返回 `503` / `429` 等错误时，会推送友好提示，随后关闭连接，前端触发重连机制
